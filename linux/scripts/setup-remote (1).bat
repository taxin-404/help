...
REM >>>> Only needed if this is Windows HOME edition <<<<
REM Pick a password for VNC remote access (max 8 characters used).
REM Leave as-is if you're on Windows Pro/Enterprise/Education.
set VNC_PASSWORD=
...
if "%VNC_PASSWORD%"=="" (
    ...
    powershell -NoProfile -ExecutionPolicy Bypass -File ".\zerossh-windows.ps1" -NetworkID "%NETWORK_ID%" -VncPassword "%VNC_PASSWORD%"
)
